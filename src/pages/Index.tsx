...
import { MapPin, ArrowRight, Mail, ExternalLink, Quote, ChevronDown, Star, Sparkles, Download, Calendar } from "lucide-react";
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-5 transition-all duration-500 ${scrolled ? 'bg-background/95 backdrop-blur-md border-b border-border' : 'bg-transparent'}`}>
      <div className="flex items-center gap-6">
        <span className="font-display font-bold text-sm tracking-wider">JAURDEN HUGHES®</span>
        <span className="hidden md:block text-muted-foreground text-sm">Toronto, Canada / {time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
      </div>
      <div className="flex items-center gap-6">
    ...
    <div className="container mx-auto px-6 md:px-12 mt-8 md:mt-12">
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-8">
        <div className="flex-1">
          <h1 className="font-display font-extrabold text-[20vw] md:text-[14vw] leading-[0.85] tracking-tighter text-foreground animate-fade-up">
            VISUAL
            <span className="inline-flex items-center ml-2">
              <span className="w-4 h-4 md:w-6 md:h-6 rounded-full bg-accent inline-block" />
              <span className="w-4 h-4 md:w-6 md:h-6 rounded-full bg-foreground/30 inline-block -ml-2" />
            </span>
          </h1>
          <h1 className="font-display font-extrabold text-[20vw] md:text-[14vw] leading-[0.85] tracking-tighter text-foreground animate-fade-up" style={{ animationDelay: '0.1s' }}>
            DESIGNER
          </h1>
        </div>
        ...
        <div className="md:max-w-sm md:pt-8 space-y-6 animate-fade-up" style={{ animationDelay: '0.2s' }}>
          <p className="text-muted-foreground font-body leading-relaxed">
            <span className="text-foreground font-medium">xotto®</span> is a leading strategic brand consultancy that positions and redefines the world's most exciting tech companies.
          </p>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project, i) => (
          <a 
            key={project.id}
            href="#"
            className="group relative overflow-hidden rounded-lg aspect-[4/5] block animate-fade-up"
      ...
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-24">
        <div>
          ...
          <h2 className="font-display font-bold text-3xl md:text-5xl leading-tight mb-4">
            from toronto - based startup to global visual design studio
          </h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              {awards.map((award, i) => (
                <a 
                  key={i}
                  href="#" 
                  className="group p-4 border border-border hover:border-accent/50 transition-all"
                >
  ...
  <div className="py-12 border-y border-border overflow-hidden bg-background">
    <div className="animate-marquee flex gap-16 whitespace-nowrap">
      {[...logos, ...logos, ...logos].map((logo, i) => (
        <span key={i} className="text-2xl font-display font-bold text-muted-foreground/30">{logo}</span>
      ))}
    </div>
  </div>
);
    ...
    <div className="container mx-auto px-6 md:px-12">
      <div className="text-center mb-16">
        ...
        <p className="text-muted-foreground mt-4 max-w-2xl mx-auto font-body">
          everything starts with thorough research and iterative experimentation.
        </p>
...
const stats = [
  ...
  { value: "25+", label: "certified award" },
];
    ...
    <div className="container mx-auto px-6 md:px-12">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {testimonials.map((testimonial, i) => (
          <div 
            key={i}
            className="p-8 md:p-10 border border-border hover:border-accent/50 transition-all group"
          >
      ...
      <div className="container mx-auto px-6 md:px-12">
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, i) => (
            <div 
              key={i}
              className="border border-border hover:border-accent/50 transition-all"
            >
              <button
...
const ContactSection = () => (
  <section id="contact" className="py-24 md:py-32 relative overflow-hidden">
    <div className="container mx-auto px-6 md:px-12">
      <div className="text-center mb-16">
        <h2 className="font-display font-bold text-4xl md:text-7xl leading-tight mb-4">
          let's make your design shine
        </h2>
        <p className="text-xl text-muted-foreground font-body max-w-xl mx-auto">
          "collaborate with me to craft exceptional designs that reflect your unique vision."
        </p>
      </div>
    ...
    <div className="container mx-auto px-6 md:px-12">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <p className="text-muted-foreground font-body text-sm">
          © 2024 Jaurden Hughes. All rights reserved.
        </p>
